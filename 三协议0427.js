...
const passWordSha224 = '61bef56c9605635be768c65e26f307c22181f65ce2c8e6e9fd74774a';
const socks5AndHttpUser = 'guanliyuan';     //socsk5和http协议用户名，设置为空即为无密码验证，需要客户端也为空
const socks5AndHttpPass = 'mimimama';    //socsk5和http协议密码，设置为空即为无密码验证，需要客户端也为空
// ---------------------------------------------------------------------------------
...
const proxyStrategyOrder = ['turn', 'socks', 'http', 'https', 'nat64'];
...
const connectViaTurnProxy = async ({hostname, port, username, password}, targetIp, targetPort) => {
    ...
    try {
        ...
        // 1. 仅释放数据通道的锁供主程序使用，保留控制通道的读写权限(cr/cw)
        dw.releaseLock();
        dr.releaseLock();

        // 2. 黑洞机制：不断读取并丢弃控制通道收到的一切响应（例如0x104心跳成功包）
        // 这一步至关重要，防止底层TCP接收缓冲区塞满导致连接被内核强制掐断
        const drainCtrl = async () => {
            try {
                while (true) {
                    const { done } = await cr.read();
                    if (done) break; 
                }
            } catch (e) {}
        };
        ...
        // 3. 突破 CF 空闲断流：每 45 秒在副通道（控制通道）发一次 Refresh (0x004)
        // 完美避开主数据通道，看视频绝不卡顿
        let refreshTimer = setInterval(async () => {
            try {
                await cw.write(await sign(stunMsg(0x004, tid(), aa)));
            } catch (err) {
                clearInterval(refreshTimer);
            }
        }, 40000);

        // 4. 接管断连回调：确保断开代理时，定时器被清理，防止 Worker 内存泄漏
        const enhancedClose = () => {
            clearInterval(refreshTimer);
            [ctrl, data].forEach(s => {try {s?.close()} catch {}});
        };

        return {readable: data.readable, writable: data.writable, close: enhancedClose, extra};
        // ====================================================================
...
const manualPipe = async (readable, writable) => {
    ...
    let globalCount = new Float64Array(11), globalBytes = new Float64Array(11);
    ...
    try {
        while (true) {
            ...
            if (statCount > 500000) {
                ...
                for (let i = 0; i < 11; i++) globalCount[i] *= 0.5, globalBytes[i] *= 0.5;
            }
